...
# MPEG-2 TS　188バイトの固定長パケットに分割されている
# 
# ES
# 映像・音声などのデータ
# 
# PSI
# ESの映像・音声データを正しく配置・解釈できるように
# データの関係性を紐付けるデータ
# 
# SI
# 番組情報や属性などのデータ
# 
# 分割されているパケットにはそれぞれ上記のデータが入っている
# パケットは先頭4バイトがヘッダ、残りがペイロード(データ領域)になっている
# ヘッダのPIDからデータ領域のデータ種別を識別する
# PID：パケットで分割されたペイロード領域のデータが何のデータかを識別する奴
# PIDはテーブルごとに固有のPIDが制定されている物もあれば
# PAT、PMT、CAT、DCTなどのテーブルから指定されるPIDもある
# 
# ペイロードは1パケットで完結するとは限らない
# もしペイロード領域が足りず、データが入りきらない
# 場合は同一PIDのもと次に送るパケットに続きのデータ
# が入れられる。
# このときヘッダーのpayload_unit_start_indicator
# が0になる。　これ重要
# 
# テーブルID
# テーブルごとに割り当てられているPIDとほぼ同一？
# ただ、PIDが同じテーブルでも区別する必要性のあるNITやSDT、EIT
# では、区別するためにこのテーブルIDが使われる
# 
# 記述子
# SIで各テーブル内において使われるデータ構造の種類みたいなの
# 番組情報などがスタッフ、リンク、シリーズなど記述子で細かくまとめられてる
# 記述子タグ
# 記述子を識別するタグ　あらかじめ決められている他独自に作ることも出来る(0x80以上0xBF以下)
# 
# 参考
# TSファイルから番組内容を取得する
# https://txqz.net/memo/2011-0523-2056.html
#
# デジタル放送に使用する番組配列情報 ARIB STD-B10 5.7版 
#
# /dev/null に消える前に… - MPEG-2 TS について
# https://aton-kish.hatenablog.jp/entry/2018/12/02/172732
# https://aton-kish.hatenablog.jp/entry/2018/12/04/004048
# https://aton-kish.hatenablog.jp/entry/2018/12/05/000318


# EIT 概要個人的まとめ
# PID 0x12 or 0x26 or 0x27で流れてくる
# パケットは大抵分割されている(入りきらないから)
# 分割されてるパケットの最初かどうかはHeaderから分かる(payload_unit_start_indicator)
# パケットの最初の場合はARIB STD-B10 第2部 P92 表5-7よりテーブルIDなどが抽出できる
# テーブルIDは別途記載されているテーブルIDの対応表より理解出来るが
# EITに関するテーブルIDは下記の通り
#   0x4E        EIT（自ストリームの現在と次の番組）
#   0x4F        EIT（他ストリームの現在と次の番組）
#   0x50 - 0x5F EIT（自ストリーム、スケジュール）
#   0x60 - 0x6F EIT（他ストリーム、スケジュール）
# 分割されてるパケットからデータを取得するには
# section_lengthよりデータの長さ(バイト数)を取得する必要がある
# section_lengthはこの項目直後からCRCまでの長さなので注意
# table_idからlast_table_idまでは11バイト
# 繰り返し内のevent_idからdescriptors_loop_lengthまでは12バイト
# descriptors_loop_length直後から繰り返しになるが
# この繰り返し内は別途記載されている記述子に関するデータが
# 入っている。
# この繰り返し内の記述子はdescriptors_loop_lengthから
# データの長さ(バイト数)を取得出来る
# この記述子の繰り返しでは、頭から
# descriptor_tag、descriptor_length
# がそれぞれ8ビットで定義されている
# おそらくdescriptor_lengthの長さで記述子ごとのデータの区切りを
# 算出して、複数の記述子のデータに対応するんじゃないか？
# 
# 拡張形式イベント記述子    P129
# 短形式イベント記述子      P141
# 
...
# 1.4.1 EIT［現在／次］の情報
# 以下の規則は、EIT［現在／次］の情報を得るのを容易にする。EIT セクションの最大
# 値は4096 バイトである。
# SI ビットストリームはサービス毎に2 つのセクションを持つ必要があり、EIT［現在／
# 次］のセクション番号0x00 を現イベントの記述のため、またセクション番号0x01 を次イ
# ベントの記述のために使用する。この制約は、ニアビデオオンデマンド（NVOD）の参照
# サービスの場合には、二つ以上のEIT［現在／次］の記述が存在する場合があるので適用
# しない。また、次のイベントより後のイベントの情報を、セクション番号0x02 以降を用い
# て任意に含ませることができる。
# SI ビットストリームでは、1 セクション中の1 つのイベントを記述するための最大値は
# 4096 バイトである。
# EIT［現在／次］は、現イベントと次イベントという概念を基本に構成されている。ど
# のイベントが現在のものであるかは、以下の体系で決定できる：
# a) どの時点においても、多くて一つの現イベントが存在する。
# b) 現イベントが存在する場合、このイベントはEIT［現在／次］のセクション0 に記
# 載される。
# c) 現イベントが存在しない場合（例えばスケジュールの隙間など）、空のEIT［現在
# ／次］のセクション0 が伝送される。
# d) 現イベントの進行状態（running_status）フィールドは、表1－1 の解釈で与えられ
# る。
#     EIT に記載されているイベント継続時間は、イベントが「非実行中」や「停止中」
#     の状態である期間も全て含まなければならない。EIT の開始時間フィールドに記載さ
#     れているイベントの開始時間は、イベント全体の開始時間でなければならない。すな
#     わち、停止期間終了後の開始時間ではない。
# e) どの時点においても、多くて1 つの次イベントが存在する。
# f) 次イベントが存在する場合、このイベントはEIT［現在／次］のセクション1 に記
# 載される。
# g) 次イベントが存在しない場合、空のEIT［現在／次］のセクション1 が伝送される。
# h) 次イベントの進行状態フィールドは、表1－2 の解釈で与えられる。
# 
#     EIT に記載されているイベント継続時間は、イベントが「非実行中」や「停止中」
#     の状態である期間も全て含まなければならない。EIT の開始時間フィールドに記載さ
#     れているイベントの開始時間は、イベント全体の開始時間でなければならない。すな
#     わち、停止期間終了後の開始時間ではない。
# 
#  あるイベントの開始時間にその継続時間を加えたものは、次イベントの開始時間より小
# さくてもよい。言い換えれば、イベント間の隙間は許容される。そのような場合、次イベ
# ントは隙間の後に開始するようにスケジュールされていると考えられる。このイベントは、
# EIT［現在／次］のセクション1 に符号化されなければならない。開始時間と継続時間は
# スケジュール化された時間である。放送事業者はスケジュールが遅れている場合には、こ
# の情報を更新してよい。一方、例えば「8 時のニュース」と呼ばれるイベントが8:00:00 開
# 始の代わりに8:01:23 開始と示されるのを避けたい場合など、示された開始時間を変更し
# ないままでもよい。
